...
export default function AdminColorsPage() {
  ...
  const loadColors = async () => {
    try {
      setLoading(true);
      const data = await adminService.getColors();
      setColors(data);
    } catch (error: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingColor) {
        await adminService.updateColor(
          editingColor.id,
          formData as UpdateColorRequest
        );
      ...
      <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-12">
            <p className="text-gray-600">Đang tải danh sách màu sắc...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Quản lý tất cả màu sắc sản phẩm</p>
          </div>
          <button
            onClick={handleAdd}
                ...
                {colors.map((color) => (
                  <tr key={color.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {color.isActive ? "Hoạt động" : "Vô hiệu hóa"}
                      </span>
                    </td>
        ...
        {colors.length === 0 && (
          <div className="text-center py-12 bg-white rounded-lg shadow">
            <p className="text-gray-600">Chưa có màu sắc nào.</p>
          </div>
        )}
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          hexCode: e.target.value.replace("#", ""),
                        })
                      }
                      className="w-16 h-12  border-gray-300 rounded-lg cursor-pointer"
                    />
                    <input
                      type="text"
                      value={formData.hexCode}
